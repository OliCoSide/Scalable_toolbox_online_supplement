...
Let \(\pi=\pi(X,D)\) denote the premium implied by the pricing function. 
We compare the \emph{distributions of premiums} between two groups; write 
\(\mu=\mathcal L(\pi\mid D=1)\) and \(\nu=\mathcal L(\pi\mid D=0)\).

\paragraph{Probabilistic definition (brief).}
For \(p\ge 1\),
\[
W_p(\mu,\nu)
=
\left(
\inf_{\substack{\Pi'\sim \mu\\ \tilde\Pi'\sim \nu}}
\ \mathbb{E}\!\left[\,\lvert \Pi'-\tilde\Pi'\rvert^{p}\right]
\right)^{1/p}.
\]
Interpretation: the smallest average \(p\)-power gap when pairing premiums from the two groups. 
Units: dollars. \(W_p=0\) iff \(\mu=\nu\).

\paragraph{Univariate formula (what we compute).}
Let \(F_\mu^{-1}\) and \(F_\nu^{-1}\) be the \emph{premium quantile functions} of the two groups. Then
\[
\boxed{\
W_p^{\,p}(\mu,\nu)
=\int_{0}^{1}\big\lvert F_\mu^{-1}(u)-F_\nu^{-1}(u)\big\rvert^{p}\,du
\ }.
\]
Useful special case \(p=1\):
\[
W_1(\mu,\nu)=\int_{0}^{1}\big\lvert F_\mu^{-1}(u)-F_\nu^{-1}(u)\big\rvert\,du
=\int_{\mathbb{R}}\lvert F_\mu(x)-F_\nu(x)\rvert\,dx.
\]
\emph{Reading:} \(W_1\) is the average absolute premium gap across matched quantiles.

\paragraph{Empirical, univariate (equal sample sizes).}
Given premiums for the two groups, sort each sample (size \(n\)):
\[
\pi^{(1)}_{(1)}\le\cdots\le \pi^{(1)}_{(n)},\qquad
\pi^{(0)}_{(1)}\le\cdots\le \pi^{(0)}_{(n)}.
\]
Then
\[
\boxed{\
\widehat W_p^{\,p}
=\frac{1}{n}\sum_{i=1}^{n}\big\lvert \pi^{(1)}_{(i)}-\pi^{(0)}_{(i)}\big\rvert^{p}\ .
\ }
\]
(With unequal weights/exposures, use weighted quantiles \(Q_1,Q_0\) and approximate 
\( \widehat W_p^{\,p}\approx \sum_k \lvert Q_1(u_k)-Q_0(u_k)\rvert^p\,\Delta u \).)
