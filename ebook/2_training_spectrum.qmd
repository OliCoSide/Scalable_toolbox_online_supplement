...
In this section, we provide an example of detailed methodology for estimating the benchmark premiums from the spectrum of fairness defined in Section 4.1 of the **main paper**. It complements the estimation of Section 4.3. 

In this paper, premiums are estimated using `lightgbm` algorithms from @Ke/al:2017, a package that supports common actuarial distributions (Tweedie, Poisson, Gamma, etc.) and was found to have excellent predictive performance compared to other boosting algorithms in @Chevalier/al:2024. Given `lightgbm`'s flexibility, we advocate for careful variable preselection to:
...
\end{equation*}
As discussed earlier, we assume $D$ is discrete or discretized. If the training sample is representative of the target population (portfolio, market, or region), empirical proportions are consistent estimators of $\Pr(D = d)$. This is also an estimator suggested in Section 5 of @Lindholm/al:2022.

```{r emp_prop}
#| code-fold: true
#| code-summary: Computing the empirical proportions for protected subpopulations
#| warning: false
...
## Computing the empirical proportions for protected subpopulations on experimental data (for later use in section 5)
...
barycenter.fit(best_est_train.values, sens_train.values)

# Load best-estimate premiums and associated sensitive attributes
best_est_test = pd.read_csv('best_est_prem_test.csv')  # Test premiums
...
# Save results
pd.DataFrame(corrective_premiums_test).to_csv('corr_prem_test.csv', index=False)
...
Suppose also that the two samples are ordered, $z_0^{(i-1)}\leq z_0^{(i)},...,z_0^{(i+1)}$ (and similarly in group $1$), then the Wasserstein distance is
...
$$
Thus, the Wasserstein distance is a distance between quantile functions $F^{-1}$, which can be extended to the case where the two samples do not have the same size.
...
One can also consider a third sample, defined as $\bar z^{(i)}=(z_{(0)}^{(i)}+z_{(1)}^{(i)})/2$. This is an ordered sample, in between the two. We can write
...
## now, the folder 'transported' exists, with one epsilon_y file per scenario
```

To predict corrective premium on other datasets, one can alternatively train a `lightgbm` model of the mapping from $(X, D)$ to the resulting corrective premiums from `Equipy` on the training sample.
...
## Visualizing the estimated spectrum

By combining all the trained models as components into a trained spectrum of fairness, we can define the premium function that will serve to predict the premiums.
...
![Estimated propensity in terms of $x_1$ and $x_2$ for simulations](figs/graph_pdx_perpop.png){#fig-prop_est width=100%}
