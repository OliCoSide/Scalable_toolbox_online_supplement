...
In order to expose systemic disparities, section 6 of the **main paper** proposes to partition policyholders following a relevant fairness local metric. In this section, we aim to perform partitioning of policyholders following proxy vulnerability (section 6.1 of the **main paper**). 

Furthermore, we perform an experiment on *Scenario 1* to see how effective partitioning is at identifying high proxy vulnerability policyholders. We gain insight into best practices for partitioning through this repeated partitioning experiment. 
...
Five essential components guide supervised partitions:
  ...
  - The **data** should represent the population for which fairness is intended, whether the full portfolio, a representative sample, or a specific subpopulation.
  - The **partitioning feature space** might include allowed and sensitive variables to reveal "minorities within minorities". Alternatively, focusing on permissible variables ensures future usability, even if sensitive attributes become inaccessible. 
...
```{r saving-dict}
#| code-fold: true
#| code-summary: Saving dictionaries of partition rules and predictions
#| warning: false
#| message: false
...
![Partition of proxy vulnerability across the three scenarios (columns) in the example. The top row compares theoretical proxy vulnerability (black) with estimated values from the lightgbm model, the latter colored by the group formed by the tree. The bottom row shows the partition in the $(x_1, x_2)$ domain, with noise added to~$x_2$ for clarity.](figs/graph_proxy_clusters_and_pop_scenario.png){#fig-ex_partitioning_clusters width=100%}


## Experiment on partitioning proxy vulnerability for scenario 1
...
kbl(table_to_g |> dplyr::select(-group), col.names = NULL,
    caption = "Results of experimental testing. For each of the $M = 100$ samples, we obtained the best regression trees when forcing $k = 8$ or when $k$ was part of the tuned hyperparameters.",
    label = "experiment") %>%
  add_header_above(c(
    "Metric" = 1,
    "Mean" = 1, "SD" = 1,
