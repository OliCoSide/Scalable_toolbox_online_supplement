...
library(latex2exp)
library(jsonlite)
library(partykit) # to predict the evtree from dictionary
```
...
summarize_fairness_table <- function(data, group_col, marg_dist,
  ...
  # edit group_col. build multiple tables: full, full-middle, pruned
  ...
  for (i in seq_along(summary_tables)) {
    ...
    for (i in seq_along(group_starts)) {
      ...
      if (i %% 2 == 0) {  # Alternate colors
        ...
        addStyle(
          wb, sheet_name, 
          style = createStyle(fgFill = "white"),  # White for odd groups
          rows = group_rows, 
          cols = 3:ncol(summary_table), 
          gridExpand = TRUE
        )
...
for (scenario in names(group_pop_stats)) {
  ...
  file_name = paste0('../docs/tables/summaries_', scenario, '.xlsx'))
}
...
<div class="d-flex flex-wrap gap-3">
  ...
  <a href="tables/summaries_Scenario3.xlsx" class="btn btn-outline-warning" role="button" download>
    ⬇ Scenario 3
  </a>
