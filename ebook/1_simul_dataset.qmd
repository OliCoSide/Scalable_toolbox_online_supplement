...
Let $X_1$ be normally distributed with mean $1$ and variance $9$. Let $X_2$ be a discrete random variable with $P(X_2 = k) = 1/4$, for $k \in \{1, 2, 3, 4\}$. Assume $D$ is a binary sensitive characteristic with $P(D = 1) = 1/2$. 
...
For scenarios 1 and 2, the propensity for $D = 1$ is given by
\begin{equation*}
...
For illustration, we assume that the claim amount is normally distributed with $\sigma^2 = 45$ and 
\begin{equation} \label{eq:ex_distY}
E(Y|X_1, X_2, D) = 100 + 4 X_1 \{(1 - \gamma_D) + \gamma_D D\} + 15 D, 
\end{equation}
with $\gamma_D = 0$ (no interaction) for scenario 1 and $\gamma_D = 0.5$ (interaction) for scenarios 2 and 3. Note that $X_2$ is not a true risk factor for $Y$ in any of the three scenarios.
...
We then define a set of parameters consistent with the three scenarios.
...
We create functions to calculate all pricing benchmarks theoretically, which involves multiple complex functions for the theoretical expression of the corrective premium.
...
#| code-fold: true
#| code-summary: Theoretical premium function definition
...
::: {.related-box}
**Related section**  
The local fairness metrics are discussed in @sec-local of this online supplement and defined in **Section 5 of the main paper**.
:::
:::
...
to_save_pdx_t_perpop <- names(df_to_g_theo)[-1] %>% 
  lapply(function(name){
    ...
    the_pops <- c(NA, 'Scenarios 1 and 2', 'Scenario 3')
...
We start with the generic simulating function, which takes the desired number of simulations `n` and a set of parameters `params` as arguments. 
...
pop_to_g_theo <- setNames(nm = names(sims)) %>% lapply(function(name) {
  ...
  message(sprintf("[%s] Step 3: Computing theoretical premiums (no muH and muC)", Sys.time()))
